...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (element) {
        ...
        ticketsList.add({
            tickets_id: `<a href="apps-tickets-overview.php" class="fw-medium link-primary">#TBS2430190${element.id}</a>`,
            assign: imgHtml,
    ...
    ticketsList.remove("tickets_id", `<a href="apps-tickets-overview.php" class="fw-medium link-primary">#TBS243019001</a>`);
    tooltipElm();
    refreshCallbacks();
    ischeckboxcheck();
}
...
Array.prototype.slice.call(forms).forEach(function (form) {
    form.addEventListener('submit', function (event) {
        ...
        if (!editList && allFieldsValid) {
            ticketsList.add({
                tickets_id: `<a href="apps-tickets-overview.php" class="fw-medium link-primary">#TBS2430190${count}</a>`,
            ...
            Array.from(editValues).forEach(function (x) {
                ...
                if (selectedId == itemId) {
                    x.values({
                        tickets_id: `<a href="apps-tickets-overview.php" class="fw-medium link-primary">${idFieldInput.value}</a>`,
                        assign: assignToUsers(),
                        ticket_title: ticketTitleInput.value,
                        client_name: clientNameInput.value,
                        create_date: createDateInput.value,
        ...
        Swal.fire({
            ...
            if (result.value) {
                for (i = 0; i < ids_array.length; i++) {
                    ticketsList.remove("tickets_id", `<a href="apps-tickets-overview.php" class="fw-medium link-primary">${ids_array[i]}</a>`);
                }
